...
Scans `fpga/src/verilog/xillydemo.v` for module instantiations that
use named port connections of the form `.port_name(signal)` and
reformats those connection lines so the `.port_name` and the
right-hand expressions inside the parentheses are column-aligned.
Preserves commas and end-of-line comments. Writes the file back if
any formatting changes were made.

Usage:
    python3 utils/align_ports.py
"""
from pathlib import Path
import re
p=Path('fpga/src/verilog/xillydemo.v')
s=p.read_text()
lines=s.splitlines()
out_lines=[]
i=0
