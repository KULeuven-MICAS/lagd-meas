...
# Then, trigger it on reboot by
# either adding it to crontab with
# `@reboot /usr/local/bin/report_ip.sh`
# or adding it to the /root/.bashrc file with
# `source /usr/local/bin/report_ip.sh` at the end.
...
# Grab the current MAC address of eth0
BOARD_MAC=$(cat /sys/class/net/eth0/address)

# If an IP exists, email it to your inbox
if [ ! -z "$BOARD_IP" ]; then
    echo "Please edit the email in file /usr/local/bin/report_ip.sh"
    # echo "The ZedBoard (lagdboard) booted successfully. Secure SSH target IP: $BOARD_IP, MAC: $BOARD_MAC" | mail -s "ZedBoard Status: Online ($BOARD_IP)" -a "From: you@email" you@email
fi
